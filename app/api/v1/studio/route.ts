...
export async function POST(request:Request) {
  try {
    ...
    const parsed=z.object({campaignId:z.string().uuid(),start:z.number().min(0),end:z.number().positive(),caption:z.string().max(200).default(''),captionStyle:z.enum(['classic','bold','signal']).default('classic')}).safeParse(await request.json().catch(()=>null));
    ...
    await rateLimit(user.user_id,'studio-render',30,3600);
    ...
    const job=await one<any>(`INSERT INTO studio_jobs(project_id,owner_id,start_seconds,end_seconds,caption,caption_style,idempotency_key) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [project.id,user.user_id,data.start,data.end,data.caption,data.captionStyle,key]);
