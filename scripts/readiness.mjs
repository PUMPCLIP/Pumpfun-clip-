...
await check('PostgreSQL migrations',async()=>{const db=new pg.Client({connectionString:process.env.DATABASE_URL});await db.connect();try{const r=await db.query('SELECT name FROM schema_migrations');for(const name of ['001_core.sql','002_studio.sql','003_rewards.sql','004_ai.sql','005_social.sql'])if(!r.rows.some(row=>row.name===name))throw new Error('Missing '+name);}finally{await db.end();}});
