...
// Enable only after developer permissions, TikTok audit and a consenting account test.
export const tiktokDirectEnabled=()=>process.env.TIKTOK_DIRECT_POST_ENABLED==='true';
export async function tiktokToken(userId:string,scope:'video.upload'|'video.publish'='video.upload'){
 ...
 const data=await response.json();if(!data.access_token||!data.refresh_token||!String(data.scope).split(',').includes(scope))throw new ApiError('TIKTOK_SCOPE_REQUIRED',403);
