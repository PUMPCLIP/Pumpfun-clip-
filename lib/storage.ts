...
export async function temporaryMediaUrl(key:string){if(!bucket)throw new Error('Private media bucket required for Meta import');return getSignedUrl(client(),new GetObjectCommand({Bucket:bucket,Key:key}),{expiresIn:3600});}
